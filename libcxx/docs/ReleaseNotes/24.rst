...
- Assigning a value to a ``basic_string`` now allocates a buffer sized for the value assigned instead of
  applying the amortized growth factor, which left the string over-allocated by up to 2x. For example,
  ``str = std::string(24, 'a')`` on a default constructed ``str`` now results in the same capacity as
  ``std::string str(24, 'a')``. Growing operations like ``append`` are unaffected and keep growing
  geometrically.
